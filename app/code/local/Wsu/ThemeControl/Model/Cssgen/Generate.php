<?php
class Wsu_ThemeControl_Model_Cssgen_Generate extends Mage_Core_Model_Abstract {
    protected $x0b;
    public function __construct() {
        parent::__construct();
        $this->_cssDirPath = Mage::getBaseDir() . '/' . Mage::helper('wsu_themecontrol')->getGeneratedCssPath();
    }
    ...
    public function generateStoreCss($x0c, $x0e = '') {
        ...
        $x15 = 'wsu/themecontrol/css/' . $x0c . '.phtml';
        ...
        try {
            ...
            if (empty($x16)) {
                throw new Exception(Mage::helper('wsu_themecontrol')->__("\124e\155\160\154\x61t\x65 f\x69l\x65 \151\163\040\145\155pty\x20or\x20d\157\x65\163\156'\x74 \x65\x78\x69\163t\072 \x25s", $x15));
            }
            ...
            $x17->streamUnlock();
            $x17->streamClose();
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('wsu_themecontrol')->__('CSS file %s has been refreshed', $x13));
        }
        catch (Exception $x18) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('wsu_themecontrol')->__('Failed refreshing css file %s in %s', $x13, Mage::helper('wsu_themecontrol')->getGeneratedCssPath()) . '<br/>Message: ' . $x18->getMessage());

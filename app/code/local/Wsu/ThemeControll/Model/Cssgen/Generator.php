<?php
class Wsu_ThemeControll_Model_Cssgen_Generator extends Mage_Core_Model_Abstract {
    ...
    protected function _generateStoreCss($x0b, $x0d) {
        ...
        $x11 = '_' . $x0d;
        $x12 = $x0b . $x11 . '.css';
        $x13 = Mage::helper('themecontroll/cssgen')->getGeneratedCssDir() . $x12;
        $x14 = 'wsu/themecontroll/css/' . $x0b . '.phtml';
        ...
        try {
            ...
            if (empty($x15)) {
                throw new Exception(Mage::helper('themecontroll')->__("Tem\x70\x6c\x61\x74e \146i\154e\x20\151\163\040\145m\160\164y \x6f\x72\x20\144\x6f\x65\x73\x6e't\x20ex\151\163\x74: \045\x73", $x14));
            }
            $x16 = new Varien_Io_File();
            $x16->setAllowCreateFolders(true);
            $x16->open(array(
                'path' => Mage::helper('themecontroll/cssgen')->getGeneratedCssDir()
            ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('themecontroll')->__('Failed generating CSS file: %s in %s', $x12, Mage::helper('themecontroll/cssgen')->getGeneratedCssDir()) . '<br/>Message: ' . $x17->getMessage());

<?php
class Wsu_ThemeControll_Helper_Labels extends Mage_Core_Helper_Abstract {
    /**
     * Get product labels
     *
     * @return string
    ...
    public function getLabels($product) {
        $html  = '';
        $isNew = false;
        if (Mage::getStoreConfig('themecontroll/product_labels/new')) {
            $isNew = $this->isNew($product);
        }
        $isSale = false;
        if (Mage::getStoreConfig('themecontroll/product_labels/sale')) {

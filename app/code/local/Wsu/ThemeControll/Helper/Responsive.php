<?php
class Wsu_ThemeControll_Helper_Responsive extends Mage_Core_Helper_Abstract {
    /**
    ...
    public function getMaxWidth($storeCode = NULL) {
        $w = Mage::helper('themecontroll')->getLayoutCfg('responsive/max_width', $storeCode);
        if ($w == 'custom') {
            return intval(Mage::helper('themecontroll')->getLayoutCfg('responsive/max_width_custom', $storeCode));
        } else {
            return intval($w);
        }
    }
